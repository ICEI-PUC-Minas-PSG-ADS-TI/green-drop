...
const CATEGORIES = [
  ...
  // { id: 'outros', name: 'Outros', icon: 'ellipsis-horizontal', description: 'Outros problemas não categorizados' }
];
...
export default function FinalScreen() {
  ...
  useEffect(() => {
    if (category && importance) {
      setPoints(calculatePoints(category.id, importance));
    } else {
      setPoints({ initial: 0, total: 0 });
    }
  }, [category, importance]);
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      //points: points.initial // Remover isso depois
    };
  ...
  // ---- JSX ----
  return (
    <ContribuirLayout currentStep={2}> <View style={styles.container}>
